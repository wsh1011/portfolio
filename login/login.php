...
<html lang="ko">
    ...
    <?
        ...
        if(!$num_match) // 검색되지 않았으면,        window생략가능(자바스크립트의 모든 메소드는 윈도우의 메소드)
        {
            echo("
                <script>
        ...
        else    // 해당 아이디가 검색되었으면
        {
            ...
            $sql ="select * from member where id='$id' and pass=password('$pass')"; // id와 password를 찾는 sql문 (pass는 암호화한 값으로 찾아야한다), password('$pass')는 1234를 패스워드 함수로 암호화한값
            $result = mysqli_query( $connect, $sql);
            $num_match = mysqli_num_rows($result);   // 검색되면 1, 없으면 0
        ...
        }//echo 안의 자바스크립트문에서 php에서만든 변수를 쓰고싶을떄 {} 함
    ?>
    <body>
    </body>
</html>
