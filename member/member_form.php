...
<head>
    ...
    $(document).ready(function() {
        ...
        //id 중복검사
        $("#id").keyup(function() {     // id입력 상자에 id값 입력시(.keyup 하면 키보드 눌렀다뗄때 이벤트발생)
            var id = $('#id').val();    //a

                $.ajax({
                    type: "POST",
                    url: "check_id.php",
                    data: "id="+ id,        // check_id.php?id= +id    비교할 수 있게 파라미터로 변수("id=")와 값(id) 보냄
                    cache: false,
                    success: function(data)                 //success: function(data) 는 비교하고 돌아오는게 완료되면 자동으로 처리되는 함수
                    {
                        //data => echo "문자열" 이 저장된
                        // console.log(data);
                        $("#loadtext").html(data);
	...
	<script>
        ...
        function check_input()
        {
            ...
            if (!document.member_form.id.value)      //필수항목처리
            {
                alert("아이디를 입력하세요");
                document.member_form.id.focus();    //커서를 id에 놔줌
                return;                       //아래코드 계산하지말고 나가라, return false;랑 같음
            } else if(document.member_form.id.value.indexOf(' ') > -1){          //입력한 아이디에 공백문자 있으면
            ...
            <form  name="member_form" method="post" action="insert.php">
                ...
                <dl>
                    ...
                    <dd>
                        ...
                        <div class="notice_txt" id="loadtext"></div> <!-- 이 빈 박스에 사용가능한 아이디입니다 씀 -->
                    </dd>
                </dl>
                <dl>
